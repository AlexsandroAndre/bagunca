 ...
 */
class cliquePost extends WP_Widget {
    /**
     * Construtor
     */
    public function cliquePost() {
        parent::__construct(false, $name = 'Inserir Post (Artigo)');
    }
    ...
    public function form($instancia) {
        ...
        <p>Post (Artigo): <select id="select-posts" name="<?php echo $this->get_field_name('select-posts'); ?>">
            <?php
            $pages = get_posts('posts_per_page=9999');
            foreach ($pages as $p) {
                $selected = '';
                if ($p->ID == $instancia['select-posts'])
                    $selected = 'selected';
                echo '<option ' . $selected . ' value="' . $p->ID . '">' . $p->post_title . '</option>';
            }
            ?>
            </select></p>
            <p>Título <span>(Coloque apenas se quiser um título diferente do título do artigo)</span>: <input class="titulo-top-artigos" name="<?php echo $this->get_field_name('titulo'); ?>" id="<?php echo $this->get_field_id('titulo'); ?>" type="text" value="<?php echo $instancia['titulo']; ?>"/></p>
...
add_action('widgets_init', create_function('', 'return register_widget("cliquePost");'));
?>